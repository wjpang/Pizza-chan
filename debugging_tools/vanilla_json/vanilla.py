...
ideas_vanilla_b4_json = MOD_PATH + r"\common\ideas\00_country_ideas.txt"
...
def ideas():
    ...
    dict_final = recursive_process_dict(dict_vanilla, localized_datas)
...
def create_localisation(loc_dir):
    ...
    for key in dict_vanilla:
        index += 1

        if "trigger" in dict_vanilla[key]:
            del dict_vanilla[key]["trigger"]
        elif "free" in dict_vanilla[key]:
            del dict_vanilla[key]["free"]

        for key_sub in dict_vanilla[key]:
            percentage = (index/(len(dict_vanilla)*11)) * 100
